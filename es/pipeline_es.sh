...
num_rhm=8000
slbfst=./FOMA/syllable_es.fst
flookup=/ixadata/soft/rhel8/bin/flookup 
mark_rhyme=mark_rhyme_es.py
...
#cat ${input}.slb | awk '{print $NF}' | stdbuf -o0 $flookup -ibx Rhyme/errimaeus.fst | awk 'NR%2==1'  > ${input}.rhm

echo "python3 $mark_rhyme  ${input}.slb ${input}.split ${output}"
python3 $mark_rhyme  ${input}.slb ${input}.split ${output}
